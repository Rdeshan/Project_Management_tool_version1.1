...
public class IndexModel(IRoleService roleService) : PageModel
{
    ...
    public async Task OnGetAsync(CancellationToken cancellationToken)
    {
        try
        {
            var allRoles = await roleService.GetAllAsync(cancellationToken);
            ...
            {
                ("Description", "desc") => allRoles.OrderByDescending(x => x.Description).ThenBy(x => x.Name),
                ("Description", _) => allRoles.OrderBy(x => x.Description).ThenBy(x => x.Name),
                ("Name", "desc") => allRoles.OrderByDescending(x => x.Name),
                _ => allRoles.OrderBy(x => x.Name)
            };
